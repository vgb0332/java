...
public class DirTreeView extends JFrame {

	private DirTreeView thisClass = this;
	private DirTreeModel model = null;
	private DirTreeController controller = null;
	 ...
	 */
	public DirTreeView() {
		this.model = new DirTreeModel(this);
		this.controller = new DirTreeController(this.model);
	...
	private void initialize(){		
		...
		//jtreeView = new JTree();
		jtreeView.setPreferredSize(new Dimension(100, 200));
		jtreeView.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
		         int selRow = jtreeView.getRowForLocation(e.getX(), e.getY());
		         TreePath selPath = jtreeView.getPathForLocation(e.getX(), e.getY());
		         if(selRow != -1) {
		...
		jtaDataView.setPreferredSize(new Dimension(200, 200));
	...
	private void openAction(){
		if (jFileChooser1.showOpenDialog(this) ==
			...
			controller.buildDirTree();
			if (selFile.isFile()){
				controller.readFile2View();
			}
			jtreeView.setModel(model.getDirTree());
		}
	}
}
